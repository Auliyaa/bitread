...
// ===========================================
// yuv422p10le
// See P210 https://docs.microsoft.com/en-us/windows/win32/medfound/10-bit-and-16-bit-yuv-video-formats?redirectedfrom=MSDN
// First plane is Y with 1 component / pixel
// Second plane is interleaved U & V with 1 component / 2px. e.g:
// U0+1,V0+1,U1+2,V1+2,...
// ===========================================
...
// yuv444p
// ===========================================
size_t yuv444p::frame_size(size_t w, size_t h)
{
  return w*h*3;
}

std::array<size_t, 3> yuv444p::plane_offsets(size_t w, size_t h)
{
  return {0,w*h,w*h*2};
}

std::array<uint8_t, 3> yuv444p::read(const uint8_t* const data, size_t x, size_t y, size_t w, const std::array<size_t,3>& planes_off)
{
  size_t xy=y*w+x;
  return {
...
void yuv444p::write(uint8_t* data, const std::array<uint8_t,3>& yuv, size_t x, size_t y, size_t w, const std::array<size_t,3>& planes_off)
{
  size_t xy=y*w+x;
